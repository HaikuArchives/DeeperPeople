...
void SplitPane::Draw(BRect /*f*/){
   ...
   if(align == B_VERTICAL){
      ...
      //FillRect(BRect(pos.x+thickness.x,Bounds().top+pad.x+1,pos.x+thickness.x,Bounds().bottom-pad.x-1));
   }else{
      SetHighColor(145,145,145);
      ...
      //FillRect(BRect(Bounds().left+pad.y+1,pos.y+thickness.y,Bounds().right-pad.y-1,pos.y+thickness.y));
   }
}
   ...
   if(PaneOne){
      if(!WinOne){
         if(align == B_VERTICAL){
            ...
            PaneOne->ResizeTo(pos.x-pad.x, Bounds().Height()-pad.x-pad.x); // widht x height
         }else{
            PaneOne->MoveTo(pad.y,Bounds().top+pad.y);
            PaneOne->ResizeTo(Bounds().Width()-pad.y-pad.y, pos.y-pad.y-pad.y); // widht x height
         }
      }
   }
   if(PaneTwo){
      if(!WinTwo){   
         if(align == B_VERTICAL){  
            ...
            PaneTwo->ResizeTo(Bounds().Width()-(pos.x+thickness.x)-pad.x, Bounds().Height()-pad.x-pad.x);
         }else{
            PaneTwo->MoveTo(Bounds().left+pad.y,pos.y+thickness.y);
            PaneTwo->ResizeTo(Bounds().Width()-pad.y-pad.y, Bounds().Height()-pos.y-pad.y-thickness.y);
         }
      }
   }
...
void SplitPane::MouseMoved(BPoint where,uint32 /*info*/,const BMessage */*m*/){
   if(Draggin){
      ...
      */
      ...
      switch(align){
         case B_HORIZONTAL:
            if (pos.y < MinSizeOne.y) pos.y = MinSizeOne.y;
            break;
         case B_VERTICAL:
            if (pos.x < MinSizeOne.x) pos.x = MinSizeOne.x;
            break;
      }
...
void SplitPane::SetState(BMessage *state){
   ...
   if(state->FindInt32("align",&i) == B_OK){
      align = i;
   }   
   if(state->FindPoint("pos",&pt) == B_OK){
      pos = pt;
   }
   if(state->FindPoint("thick",&pt) == B_OK){
      thickness = pt;  
   }
   if(state->FindPoint("jump",&pt) == B_OK){
      jump = pt;
   }
   if(state->FindPoint("pad",&pt) == B_OK){
      pad = pt;
   }
   if(state->FindPoint("minsizeonw",&pt) == B_OK){
      MinSizeOne = pt;
   }
   if(state->FindPoint("minsizetwo",&pt) == B_OK){
      MinSizeTwo = pt;
   }
   ...
   if(state->FindBool("resizeonex",&resizeOneX) != B_OK){
      resizeOneX = false;
   }
   if(state->FindBool("resizeoney",&resizeOneY) != B_OK){
      resizeOneY = false;
   }
